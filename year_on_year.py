...
def year_on_year_page():
    ...
    if show_filters:
        ...
        year_cols = st.columns([1, 1, 1, 3])  # The last column is for spacing
        
        # Initialize year filter state if not exists
        if 'selected_years' not in st.session_state:
            st.session_state.selected_years = ['All']
        
        # Year filter checkboxes
        with year_cols[0]:
            if st.checkbox("All", value='All' in st.session_state.selected_years, key="year_all"):
                st.session_state.selected_years = ['All']
        with year_cols[1]:
            if st.checkbox("2023", value='2023' in st.session_state.selected_years, key="year_2023"):
                if 'All' in st.session_state.selected_years:
                    st.session_state.selected_years = ['2023']
                elif '2023' not in st.session_state.selected_years:
                    st.session_state.selected_years.append('2023')
            elif '2023' in st.session_state.selected_years:
                st.session_state.selected_years.remove('2023')
        with year_cols[2]:
            if st.checkbox("2024", value='2024' in st.session_state.selected_years, key="year_2024"):
                if 'All' in st.session_state.selected_years:
                    st.session_state.selected_years = ['2024']
                elif '2024' not in st.session_state.selected_years:
                    st.session_state.selected_years.append('2024')
            elif '2024' in st.session_state.selected_years:
                st.session_state.selected_years.remove('2024')
        
        # If no years are selected, select All
        if len(st.session_state.selected_years) == 0:
            st.session_state.selected_years = ['All']
        
        # Apply year filter
        if 'All' not in st.session_state.selected_years:
            filtered_df = filtered_df[filtered_df['Year'].astype(str).isin(st.session_state.selected_years)]
